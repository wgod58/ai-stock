    ...
    if slope:
        ...
        for idx in trange(len(data)):
            if data.loc[idx, "direction"] == 1:
                data.loc[idx, "ushadow_width"] = (
                    data.loc[idx, "high"] - data.loc[idx, "close"]
                )
                data.loc[idx, "lshadow_width"] = (
                    data.loc[idx, "open"] - data.loc[idx, "low"]
                )
            else:
                data.loc[idx, "ushadow_width"] = (
                    data.loc[idx, "high"] - data.loc[idx, "open"]
                )
                data.loc[idx, "lshadow_width"] = (
                    data.loc[idx, "close"] - data.loc[idx, "low"]
                )

            if idx <= 50:
                data.loc[idx, "body_per"] = stats.percentileofscore(
                    abs(data["diff"]), abs(data.loc[idx, "diff"]), "rank"
                )
                data.loc[idx, "upper_per"] = stats.percentileofscore(
                    data["ushadow_width"], data.loc[idx, "ushadow_width"], "rank"
                )
                data.loc[idx, "lower_per"] = stats.percentileofscore(
                    data["lshadow_width"], data.loc[idx, "lshadow_width"], "rank"
                )
            else:
                data.loc[idx, "body_per"] = stats.percentileofscore(
                    abs(data.loc[idx - 50 : idx, "diff"]),
                    abs(data.loc[idx, "diff"]),
                    "rank",
                )
                data.loc[idx, "upper_per"] = stats.percentileofscore(
                    data.loc[idx - 50 : idx, "ushadow_width"],
                    data.loc[idx, "ushadow_width"],
                    "rank",
                )
                data.loc[idx, "lower_per"] = stats.percentileofscore(
                    data.loc[idx - 50 : idx, "lshadow_width"],
                    data.loc[idx, "lshadow_width"],
                    "rank",
                )
...
def detect_evening_star(data, q=None, multi=False, short_per=35, long_per=65):
    ...
    try:
        for idx in tqdm(temp):
            ...
            cond4 = (
                data.loc[idx + 1, "close"] + data.loc[idx + 1, "open"]
            ) / 2 >= data.loc[idx, "close"]
            cond5 = data.loc[idx + 2, "close"] <= (
                (data.loc[idx, "open"] + data.loc[idx, "close"]) / 2
            )
            # cond6 = (data.loc[idx+2, 'body_per'] >= long_per)
            cond7 = (
                data.loc[idx + 2, "open"]
                <= (data.loc[idx + 1, "open"] + data.loc[idx + 1, "close"]) / 2
            )
...
def detect_shooting_star(data, q=None, multi=False, short_per=35, long_per=65):
    ...
    try:
        for idx in tqdm(temp):
            ...
            cond3 = data.loc[idx + 1, "ushadow_width"] > 2 * abs(
                data.loc[idx + 1, "diff"]
            )
            cond4 = min(data.loc[idx + 1, "open"], data.loc[idx + 1, "close"]) > (
                (data.loc[idx, "close"] + data.loc[idx, "open"]) / 2
            )
...
def detect_inverted_hammer(data, q=None, multi=False, short_per=35, long_per=65):
    ...
    try:
        for idx in tqdm(temp):
            ...
            cond4 = max(data.loc[idx + 1, "open"], data.loc[idx + 1, "close"]) < (
                (data.loc[idx, "close"] + data.loc[idx, "open"]) / 2
            )
...
def detection_result(data):
    ...
    print(
        "[ Info ] : number of bullish engulfing is %s"
        % np.sum(data["bullish_engulfing"])
    )
    print(
        "[ Info ] : number of bearish engulfing is %s"
        % np.sum(data["bearish_engulfing"])
    )
    print("[ Info ] : number of hammer is %s" % np.sum(data["hammer"]))
    print(
        "[ Info ] : number of inverted hammer is %s" % np.sum(data["inverted_hammer"])
    )
...
if __name__ == "__main__":
    ...
    # load raw ohlc data
    data = pd.read_csv("./data/BTCUSDT_1h_train.csv")
    print("Hello")
    ...
    # # save current data
    # data.to_csv('./data/adusd_2010_2017_process.csv', index=False)

    # # detect with customized rules
    data = detect_all(data, TASLS_LS, multi=True, pro_num=4)
    data.to_csv("./data/BTCUSDT_1h_train_pattern.csv", index=False)
